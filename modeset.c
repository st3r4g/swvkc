...
uint32_t modeset_get_width() { return modeset_list->width; }
uint32_t modeset_get_height() { return modeset_list->height; }
...
void modeset_copy(uint32_t stride, uint32_t width, uint32_t height, uint8_t* m) {
	...
	for (int j = 0; j < height; ++j) {
		unsigned int screen_off = iter->stride * j;
		unsigned int off = stride * j;
		memcpy(iter->map+screen_off, m+off, width * 4);
	}
}
